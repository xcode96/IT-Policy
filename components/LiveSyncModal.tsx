...
const LiveSyncModal: React.FC<LiveSyncModalProps> = ({ onClose, onSync, onDisconnect, syncStatus, syncUrl }) => {
    ...
    const renderStatus = () => {
        if (currentStatus === 'connected') {
            return <p className="text-sm text-green-600 truncate">Connected to: <span className="font-medium">{syncUrl}</span></p>;
        }
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-between items-center gap-3 mt-8">
                        <div>
                             {currentStatus === 'connected' && (
                                <button
                                    ...
                                    className="px-5 py-2 text-sm font-semibold text-red-700 bg-red-100 rounded-md hover:bg-red-200 transition-all duration-300"
                                >
                                    Disconnect
                                </button>
                            )}
                        ...
                        <div className="flex gap-3">
                            ...
                            <button 
                                type="submit" 
                                className="px-5 py-2 w-40 text-sm font-semibold text-white bg-primary rounded-md hover:bg-primary-light focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-surface focus:ring-primary disabled:bg-slate-400 disabled:cursor-wait disabled:shadow-none transition-all duration-300 shadow-sm"
