  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-center">
          <LoadingSpinner />
          <p className="mt-4 text-textPrimary font-semibold">Generating policy content...</p>
          <p className="mt-2 text-sm text-textSecondary">Please wait a moment while we consult our digital archives.</p>
        </div>
      );
    }
    ...
    if (content) {
      if (isStaticContent) {
         return (
            <article>
              <h1 className="text-3xl font-bold text-textPrimary tracking-tight mb-6">{policy.name}</h1>
              <div className="bg-slate-50 p-6 rounded-lg border border-border">
                <pre className="whitespace-pre-wrap font-sans text-sm text-textSecondary leading-relaxed">{content}</pre>
              </div>
            </article>
        );
      }
