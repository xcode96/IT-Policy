...
const PolicyDetail: React.FC<PolicyDetailProps> = ({ 
  ...
  const headerContent = (
     <header className="flex-shrink-0 border-b border-border bg-surface/80 backdrop-blur-sm">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-3">
            {isAdmin ? (
              <button 
                onClick={onLogout} 
                className="px-5 py-2 text-sm font-semibold rounded-lg border border-border text-textSecondary bg-surface hover:bg-gray-100 hover:text-textPrimary transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-dark"
              >
                Logout
              </button>
            ) : (
              <button 
                ...
                className="px-5 py-2 text-sm font-semibold text-white bg-primary rounded-lg hover:bg-primary-dark transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-dark shadow-sm hover:shadow-md"
              >
                Admin Login
              </button>
            )}
  ...
  const mainContent = () => {
    ...
    if (!policy) {
        return (
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              <div className="p-5 bg-primary rounded-full text-white">
    ...
    if (isEditing) {
      return (
        <div className="h-full flex flex-col p-6 md:p-8 lg:p-10">
          <h2 className="text-2xl font-bold text-textPrimary mb-4 flex-shrink-0">Editing: <span className="text-primary-dark">{policy.name}</span></h2>
          <textarea
            className="w-full flex-grow p-4 font-mono text-sm border border-border rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition bg-gray-50 text-gray-800"
            value={editedContent}
    ...
    return (
      <div className="h-full p-6 md:p-8 lg:p-10">
              ...
              {isAdmin && (
                  <div className="flex items-center gap-3 flex-shrink-0">
                      <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg border border-border text-textSecondary bg-surface hover:bg-gray-100 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 disabled:opacity-50 disabled:cursor-wait"
                      >
                          {isExportingSingleJson ? (
                              <>
                       ...
                       <button 
                          ...
                          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-primary-dark bg-amber-100 rounded-lg hover:bg-amber-200 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-dark"
                      >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
          ...
          {content ? (
              <article 
                className="prose prose-sm sm:prose-base max-w-none text-textSecondary prose-headings:text-textPrimary prose-headings:font-semibold prose-a:text-primary-dark hover:prose-a:text-primary-dark prose-strong:text-textPrimary prose-blockquote:border-primary prose-code:text-primary-dark prose-li:marker:text-primary prose-th:text-textPrimary prose-th:font-semibold prose-td:border-border prose-thead:border-b-2 prose-thead:border-border" 
                dangerouslySetInnerHTML={{ __html: marked.parse(content) }} 
              />
          ) : (
              <div className="flex flex-col items-center justify-center text-center p-8 border-2 border-dashed border-border rounded-lg bg-gray-50/50 mt-8">
